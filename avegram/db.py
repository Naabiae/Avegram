...
def _get_pool() -> ConnectionPool:
    ...
    if _pool is None or _pool.closed:
        ...
        _pool = ConnectionPool(
            ...
            min_size=1,
            max_size=5,
            kwargs={"connect_timeout": 10, "autocommit": False},
        )
    return _pool


def db_conn():
    """Return a pooled connection (caller must use as context manager or call .close())."""
...
def db_init():
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur:
            ...
            """)
            # Pending retry store: short token → full trade params
            cur.execute("""
...
def db_save_pending_retry(retry_key: str, telegram_id, chain, assets_id, in_token, out_token, in_amount, swap_type):
    """Store full trade params under a short key for the retry callback."""
    try:
        with _get_pool().connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO pending_retries (retry_key, telegram_id, chain, assets_id, in_token, out_token, in_amount, swap_type)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                    ON CONFLICT (retry_key) DO UPDATE SET
                        telegram_id = EXCLUDED.telegram_id,
                        chain = EXCLUDED.chain,
                        assets_id = EXCLUDED.assets_id,
                        in_token = EXCLUDED.in_token,
                        out_token = EXCLUDED.out_token,
                        in_amount = EXCLUDED.in_amount,
                        swap_type = EXCLUDED.swap_type,
                        created_at = now()
                    """,
                    (retry_key, str(telegram_id), chain, assets_id, in_token, out_token, str(in_amount), swap_type),
                )
            conn.commit()
    except Exception:
        pass
...
def db_get_pending_retry(retry_key: str):
    """Retrieve full trade params for a retry key. Returns dict or None."""
    try:
        with _get_pool().connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute("SELECT * FROM pending_retries WHERE retry_key = %s", (retry_key,))
                row = cur.fetchone()
